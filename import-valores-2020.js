...
const NOME_EDICAO = '10ª EDIÇÃO - 2020';

// Converte números em formato brasileiro (vírgula decimal, mistura com ponto) para Number
...
async function importarValores2020() {
  // Busca o id da edição 2020 e a tabela de porte já cadastrada para ela
  const { rows: edicaoRows } = await pool.query('SELECT id FROM edicoes WHERE nome = $1', [NOME_EDICAO]);
  if (edicaoRows.length === 0) {
    throw new Error(`Edição "${NOME_EDICAO}" não encontrada. Rode o setup-edicao-2020.js primeiro.`);
  }
  ...
  const valorPorte = new Map(portesRows.map((p) => [p.porte, Number(p.valor)]));
  const valorUco = valorPorte.get('UCO') || 0;

  const workbook = XLSX.readFile(ARQUIVO);
  const sheet = workbook.Sheets[ABA];
  const dados = XLSX.utils.sheet_to_json(sheet, { defval: null });

  console.log(`Lidos ${dados.length} registros. Iniciando cálculo e importação...`);
  ...
  for (const linha of dados) {
    ...
    if (!codigoRaw || !descricao || isNaN(codigo)) {
      ignorados++;
      continue;
    }

    const porte = (linha['Porte do Procedimento CBHPM'] ?? '').toString().trim() || null;

    // Fração: se vier 0/vazio, assume 1 (valor cheio do porte), conforme decidido
    let fracao = parseNumeroBR(linha['Fração do porte CBHPM']);
    if (fracao === 0) fracao = 1;

    const valorPorteUnitario = porte ? (valorPorte.get(porte) || 0) : 0;
    const totalPorte = fracao * valorPorteUnitario;

    const uco = parseNumeroBR(linha['Custo Operacional CH']);
    const totalUco = uco * valorUco;

    const filme = parseNumeroBR(linha['Filme(m2)']);
    const numeroAuxiliares = parseNumeroBR(linha['QTD. Auxiliares']);
    const incidencias = parseNumeroBR(linha['Nº de Incidências']);
    const porteAnestesico = linha['Cód. Porte Anestésico']
      ? linha['Cód. Porte Anestésico'].toString().trim()
      : null;

    // Porte Anestésico e Auxiliares sem valor monetário por enquanto (não temos a tabela de referência 2020)
    const subtotal = totalPorte + totalUco;

    cols.codigo.push(codigo);
    cols.edicao_id.push(edicaoId);
    cols.descricao.push(descricao.toString().trim());
  ...
  console.log(`\nImportação concluída! ${cols.codigo.length} registros inseridos (${ignorados} ignorados por código inválido).`);
  await pool.end();
}

importarValores2020().catch((err) => {
